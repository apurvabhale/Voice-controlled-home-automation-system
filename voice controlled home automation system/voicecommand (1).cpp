...
int main(int argc, char* argv[]) {
    ...
    if(vc.edit) {
        ...
        float volume = 0.0f;
        changemode(1);
        string cont_com = "curl -X POST --data-binary @/dev/shm/noise.flac --user-agent 'Mozilla/5.0' --header 'Content-Type: audio/x-flac; rate=44100;' 'https://www.google.com/speech-api/v2/recognize?output=json&lang=" + vc.lang + "&key=AIzaSyBOti4mM-6x9WDnZIjIeyEU21OpBXqWBgw&client=Mozilla/5.0' | sed -e 's/[{}]/''/g' | awk -F\":\" '{print $4}' | awk -F\",\" '{print $1}' | tr -d '\\n'";
        while(vc.continuous) {
            volume = GetVolume(vc.recordHW, vc.command_duration, true);
		        if(volume > vc.thresh) {
		                //printf("Found volume %f above thresh %f\n",volume,vc.thresh);
		                if(vc.verify) {
		                    system("flac /dev/shm/noise.wav -f --best --sample-rate 44100 -o /dev/shm/noise.flac 1>>/dev/shm/voice.log 2>>/dev/shm/voice.log");
...
switch ( i )
{
case 11 : temp_json["state"]="on";
	    temp_json["light"]="switch1b";
            break;

case 12 : temp_json["state"]="on";
	    temp_json["light"]="switch2a";
            break;

case 13 : temp_json["state"]="off";
	    temp_json["light"]="switch1b";
            break;

case 14 : temp_json["state"]="off";
	    temp_json["light"]="switch2a";
            break;
case 15 : temp_json["state"]="on";
	  temp_json["light"]="switch1c";
           break;

case 16 : temp_json["state"]="off";
	    temp_json["light"]="switch1c";
           break;

case 17 : temp_json["state"]="on";
	    temp_json["light"]="switch2b";
		Speak("four");
           break;

case 18 : temp_json["state"]="off";
	    temp_json["light"]="switch2b";
		Speak("five");
           break;

case 19 : temp_json["state"]="on";
	    temp_json["light"]="switch2c";
           break;

case 20 : temp_json["state"]="on";
	    temp_json["light"]="switch2c";
           break;

case 21 : temp_json["state"]="off";
	    temp_json["light"]="switch2c";
           break;

case 22 : temp_json["state"]="off";
	    temp_json["light"]="switch2c";
           break;


case 23 : temp_json["state"]="on";
	    temp_json["light"]="switch1a";
           break;

case 24 : temp_json["state"]="off";
	    temp_json["light"]="switch1a";
		Speak("ten");
           break;

case 25 : temp_json["state"]="on";
	    temp_json["light"]="switch3a";
           break;

case 26 : temp_json["state"]="off";
	    temp_json["light"]="switch3a";
           break;

case 27 : temp_json["state"]="on";
	    temp_json["light"]="all";
           break;

case 28 : temp_json["state"]="off";
	    temp_json["light"]="all";
           break;

case 29 : temp_json["state"]="on";
	    temp_json["light"]="switch1d";
           break;

case 30 : temp_json["state"]="off";
	    temp_json["light"]="switch1d";
           break;
...
std::string output=fastWriter.write(temp_json);
curl_easy_setopt(curl,CURLOPT_HTTPHEADER,headers);
 if (i == 27 or i == 28)
   curl_easy_setopt(curl,CURLOPT_URL,"http://192.168.1.108:3000/updateAllLight/");
 else
   curl_easy_setopt(curl,CURLOPT_URL,"http://192.168.1.108:3000/updateLight/");
...
std::string curl_opt = response.c_str();
...
//
		...
		{
	          if(curl_opt=="redundanton")
			system("tts It is already on");	
 		  else if(curl_opt=="redundantoff")
		      system("tts It is already off");
		  else
		  {
		    printf("command: %s\n",commands[i].c_str());
    ...
    if(message != NULL && !checkit.empty()) {
        fprintf(stderr,"Attempting to answer XXX: %s\n",message);
        Init();
        Search(message);
    } else if(!passthrough) {
...
string command = "tts -l " + lang + " ";
    command += message;
    command += "\" 1>>/dev/shm/voice.log 2>>/dev/shm/voice.log";
...
int VoiceCommand::Search(const char* search) {
    ...
    if(api.empty()) {
        ...
        if (regex_search(curlbuf.c_str(), m, rexp)) {
            ...
            printf("Could not find answer. Try again 1.\n");
        ...
        if (1) {
	        ...
	        if (result.empty()) {
	            ...
	            printf("Could not find answer. Try again 2.\n");
                Speak("Could not find answer. Try again");
	        } else {
        ...
        && regex_search(m[1].second, endbuf, m, rexp)) {
                ...
                printf("Could not find answer. Try again 3.\n");
                Speak("Could not find answer. Try again");
                return -1;
            }
        }
